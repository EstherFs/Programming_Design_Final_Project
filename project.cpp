...
int check(account *NewNode) {
    ...
    for (curperson = head; curperson != NULL; curperson = curperson->NextPerson) {
        for (cur = curperson; cur != NULL; cur = cur->next) {
            if (NewNode->name != cur->name) continue;
            if (strcmp(NewNode->classes, cur->classes) == 0 && strcmp(NewNode->item, cur->item) == 0 &&
                NewNode->month == cur->month && NewNode->classes == cur->classes)
                flag = 0;
        }
    }
    return flag;
...
void sortedDateup(account *newnode) {
    if (sortDate == NULL || ((sortDate->month) * 100 + (sortDate->day)) >= ((newnode->month) * 100 + (newnode->day))) {
        ...
        while (current->next != NULL &&
               ((current->next->month) * 100 + (current->next->day)) < ((newnode->month) * 100 + (newnode->day))) {
            current = current->next;
        }
        newnode->next = current->next;
        current->next = newnode;
...
void sort_traverse(void) {
    account *current = sortHead;
    while (current != NULL) {
        printf("%s %s %s %d/%d %d\n", current->name, current->classes, current->item, current->month, current->day,
               current->price);
        current = current->next;
    }
}